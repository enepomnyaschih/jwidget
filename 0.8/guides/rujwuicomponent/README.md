...
Если метод возвращает JW.UI.Component или JW.AbstractArray, то результат будет трактоваться как дочерний компонент
...
Для создания массивов UI компонентов на основе массивов данных удобно использовать JW.AbstractCollection.Mapper.
Благодаря ему, представление будет автоматически обновляться при изменении данных.

По этой же причине рекомендуем использовать JW.AbstractCollection в реализации классов модели вместо нативных
...
- Если метод возвращает JW.UI.Component, то он будет добавлен в словарь {@link JW.UI.Component#children children} и станет дочерним компонентом.
- Если метод возвращает JW.AbstractArray, то он будет добавлен как массив дочерних компонентов методом {@link JW.UI.Component#addArray addArray}.
