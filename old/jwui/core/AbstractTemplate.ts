...
module JW {
	export module UI {
		 ...
		 */
		export class AbstractTemplate extends JW.Class {
			/**
			 * @hidden
			 */
			...
			_compileAttributes(root: HTMLElement) {
				this.prefixes = String.parseClass(root.getAttribute("jwclass"));
				root.removeAttribute("jwclass");
				for (var i = 0, l = this.prefixes.length; i < l; ++i) {
					addClass(root, this.prefixes[i]);
				}
			...
			_walkAll(root: HTMLElement) {
				this._walk(root, [], [], (el: HTMLElement, path: number[]): string[] => {
					...
					var ids = String.parseClass(attr);
					...
					for (var i = 0; i < l; ++i) {
						...
						for (var j = 0, n = this.prefixes.length; j < n; ++j) {
							addClass(el, this.prefixes[j] + "-" + id);
						}
